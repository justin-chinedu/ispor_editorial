...
export const QuestionForm = () => {
    ...
    type ButtonText = "Send Anonymously" | "Please Wait ..." | "Sent Successfuly";
    const [buttonText, setbuttonText] = useState<ButtonText>("Send Anonymously");
    ...
    useEffect(() => {
        switch (uploadState.state) {
            ...
            case "error":
            case "idle":
                setbuttonText("Send Anonymously");
                break;
            case "success":
    ...
    const handleInput: React.ChangeEventHandler<HTMLTextAreaElement> = (e) => {
        ...
        if (buttonText !== "Send Anonymously") {
            setbuttonText("Send Anonymously");
        }
    ...
    const disabled = question.trim().length < 1 || uploadState.state == "uploading";
    ...
    return (
        <div className="w-full">
            <form className="w-full flex flex-col gap-y-4" action="">
                ...
                <button disabled={disabled} type="button" onClick={handleSubmit} className={`${uploadState.state === "success" && buttonText != "Send Anonymously" ? "bg-emerald-500" : "bg-neutral-700/40"} py-2 px-4 rounded-lg ${disabled ? "opacity-60" : "hover:bg-gray-700"}`}>
                    <div className={`flex items-center gap-x-2 justify-center ${uploadState.state === "success" ? "text-white" : "text-primary-color"}`}>
                        ...
                        {uploadState.state === "uploading"
                            ...
                            : (uploadState.state === "success") && buttonText != "Send Anonymously" ? <CheckCircle /> : <Send />
                        }
                    </div>
                </button>
                {
